using BinarySerializer.Ubisoft.GbaEngine;
using GbaMonoGame.AnimEngine;

namespace GbaMonoGame.Rayman3;

public class BossScalemanBar : Bar
{
    public BossScalemanBar(Scene2D scene) : base(scene) { }

    public AnimatedObject BossHealthBar { get; set; }
    public int BossDamage { get; set; }

    public override void Load()
    {
        AnimatedObjectResource resource = Rom.Loader.ReadResource<AnimatedObjectResource>(Rayman3DefinedResource.BossScalemanBarAnimations);

        BossHealthBar = new AnimatedObject(resource, false)
        {
            IsFramed = true,
            CurrentAnimation = BossDamage,
            ScreenPos = new Vector2(-60, 32),
            HorizontalAnchor = HorizontalAnchorMode.Right,
            BgPriority = 0,
            ObjPriority = 0,
            RenderContext = Scene.HudRenderContext,
        };

        // Re-position the health bar in time attack to avoid the UI overlapping
        if (Rayman3.TimeAttack.IsActive)
        {
            BossHealthBar.VerticalAnchor = VerticalAnchorMode.Bottom;
            BossHealthBar.ScreenPos = BossHealthBar.ScreenPos with { Y = -8 };
        }
    }

    public override void Set()
    {
        BossDamage++;
        BossHealthBar.CurrentAnimation = BossDamage;
    }

    public override void Draw(AnimationPlayer animationPlayer)
    {
        if (Mode == BarMode.StayHidden) 
            return;
        
        animationPlayer.PlayFront(BossHealthBar);
    }
}